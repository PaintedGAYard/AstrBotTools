...
public class AddAstrBotKnowledgeBaseDocument : PSCmdlet, IDisposable
{
    ...
    private readonly List<Task<UploadResult>> _runningTasks = new();
    ...
    protected override void EndProcessing()
    {
        ...
        try
        {
            Task.WaitAll(_runningTasks.ToArray(), _cts.Token);
        }
    ...
    {
        while (_runningTasks.Count >= UploadBatchSize)
        {
            int doneIndex = Task.WaitAny(_runningTasks.ToArray(), _cts.Token);
            if (doneIndex < 0) break;

            // 收集所有已完成的 Task
            var completed = _runningTasks.Where(t => t.IsCompleted).ToArray();
